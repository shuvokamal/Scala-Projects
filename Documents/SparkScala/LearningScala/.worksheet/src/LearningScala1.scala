object LearningScala1 {;import org.scalaide.worksheet.runtime.library.WorksheetSupport._; def main(args: Array[String])=$execute{;$skip(129); 
  ...
  val isBest: Boolean = picard == bestCaptain;System.out.println("""isBest  : Boolean = """ + $show(isBest ));$skip(149); 
  
  // EXERCISE
  // Write some code that takes the value of pi, doubles it, and then prints it within a string with
  val DoublePi : Double = pi*2;System.out.println("""DoublePi  : Double = """ + $show(DoublePi ));$skip(49); 
  println(f" Double Pi is about: $DoublePi%.4f")}
